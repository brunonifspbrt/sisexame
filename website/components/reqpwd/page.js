...
export default function UsuarioReqPwd({ limpaTela, usuarioID }) {
  ...
  const handleSenha = async () => {
    ...
    const dados = {
      ...
      // urlSite: `${window.location.origin}/resetpwd/${id}`, // adiciona o caminho do site para resetar senha
    };
    // console.log(dados);
  ...
  return (
    <Modal show={modalOpen} onClose={handleClose}>
      ...
      <ModalBody className="dark:text-white">
        Deseja realmente solicitar a redefinição de sua senha?
      </ModalBody>
