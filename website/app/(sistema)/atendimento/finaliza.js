...
import { Finalizar } from "./api";
import { toast } from "react-toastify";

export default function Finaliza({ idAgenda, nome, dtAgend }) {
  ...
  return (
    <Modal show={modalOpen} onClose={handleClose}>
      ...
      <ModalBody className="dark:text-white">
        Confirma a finalização do exame de {` "${nome}"`} agendado às{" "}
      ...
      <ModalFooter className="justify-end">
        ...
        >
          {busy ? (
            ...
            <HiOutlineShieldCheck className="mr-1 h-5 w-5" />
          )}
          Finalizar
        </Button>
        <Button
