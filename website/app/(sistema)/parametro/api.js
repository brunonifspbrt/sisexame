...
export async function Inserir(data) {
  ...
  const url = process.env.API_URL + "/parametro";
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      retorno.message = "Parâmetro salvo com sucesso";
    } else {
...
export async function BuscarRegistro(id) {
  ...
  const url = process.env.API_URL + "/parametro/" + id;
...
export async function Atualizar(data) {
  ...
  // console.log(args);
  const url = process.env.API_URL + "/parametro/" + data.id;
  ...
  try {
    ...
    if (result.status === 200) {
      //ações em caso de sucesso
      retorno.success = true;
      retorno.message = "Parâmetro atualizado com sucesso";
    } else {
      //ações em caso de erro
      let errorMessage = "";
      if (resultData.errors != null) {
