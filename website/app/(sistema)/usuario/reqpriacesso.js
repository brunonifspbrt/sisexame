...
import { EnviarEmailAcesso } from "./api";
import { toast } from "react-toastify";
import { useRouter } from "next/navigation";
import { UsuarioContext } from "./context";

export default function ReqAcessoInicial({ id, nome }) {
  ...
  const handleEmail = async (codUsuario) => {
    ...
    const resultado = await EnviarEmailAcesso(dados);
    ...
    if (resultado.success) {
      ...
      if (resultado.message !== "")
        toast.success("Usuário: " + resultado.message);
    } else {
      if (resultado.message !== "")
  ...
  return (
    <Modal show={modalOpen} onClose={handleClose}>
      <ModalHeader className="dark:text-white">
        Reenvio de E-mail (1º Acesso)
      </ModalHeader>
      <ModalBody className="dark:text-white">
        Deseja realmente solicitar o reenvio do e-mail de primeiro acesso para{" "}
        {nome}?
      </ModalBody>
