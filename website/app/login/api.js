...
export async function Autenticar(data) {
  ...
  const url = process.env.API_URL + "/login/autenticar";
...
export async function GeraInfoUsuario(data) {
  const args = {
    method: "GET",
    headers: {
      "x-api-key": process.env.API_KEY,
  ...
  // console.log("Valor do e-mail");
  // console.log(data);
  // console.log(data.email);
  const url = process.env.API_URL + "/login/infousuario/" + data?.email;
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      retorno.success = true;
      retorno.data = resultData;
      retorno.message = "Informação obtida com sucesso";
      //   resultData.name = resultData.nome; //pra armazenar no cookie
      //   retorno.data = resultData;
    } else {
      //ações em caso de erro
